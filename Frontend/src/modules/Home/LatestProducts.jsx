         ...
         <h2 className="text-xl font-semibold mb-2">🛒 New Arrivals</h2>
          <div className="flex gap-3 overflow-x-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-800">
            {latestProducts.map((product) => (
              <div key={product._id} onClick={()=>navigate(`/product/${product._id}`)} className="flex-shrink-0 w-36 sm:w-40 md:w-44 bg-[#1e1e1e] rounded-xl shadow-md p-3 mx-2">
              {!product.imageUrl ? (
              ...
              <div className="mt-3 space-y-1">
                ...
                onClick={(e)=>{
                  e.stopPropagation();
                  handleAddToWishlist(product._id,product.name,product.imageUrl,product.brand,product.price)
                  }}>
                  Add to Wishlist
                </button>
              </div>
